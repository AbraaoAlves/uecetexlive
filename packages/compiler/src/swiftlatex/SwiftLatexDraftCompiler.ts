...
async function loadEngineScript(
...
): Promise<NonNullable<typeof window.PdfTeXEngine>> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    target.addEventListener("error", () =>
      reject(new Error(`Failed to load ${engineScriptUrl}`)),
    );
  });
...
export class SwiftLatexDraftCompiler implements PdfCompiler {
  ...
  /** @param assetBaseUrl URL (com barra final) dos assets AGPL vendorizados da SwiftLaTeX. */
  ...
  private async doWarmup(onProgress?: WarmupProgressFn): Promise<void> {
    ...
    const EngineCtor = await loadEngineScript(`${this.assetBaseUrl}PdfTeXEngine.js`);
    this.engine = new EngineCtor();
    await this.engine.loadEngine();
    // Barra final importa: o worker concatena <endpoint>pdftex/<fmt>/<arquivo>.
    this.engine.setTexliveEndpoint(`${this.assetBaseUrl}texlive/`);
