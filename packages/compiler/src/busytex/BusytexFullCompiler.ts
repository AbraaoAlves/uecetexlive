...
export class BusytexFullCompiler implements PdfCompiler {
  ...
  /** @param assetBaseUrl URL (com barra final) dos assets vendorizados do busytex. */
  ...
  private async doWarmup(onProgress?: WarmupProgressFn): Promise<void> {
    const manifest: { files: ManifestAsset[] } = await (
      await fetch(`${this.assetBaseUrl}manifest.json`)
    ).json();
    ...
    const workers = Array.from({ length: 4 }, async () => {
      while (true) {
        ...
        if (!asset) return;
        await fetchWithProgress(this.assetBaseUrl + asset.name, (n) => {
          loaded += n;
    ...
    const injectNames: string[] = await (
      await fetch(`${this.assetBaseUrl}inject/manifest.json`)
    ).json();
    this.injectFiles = await Promise.all(
      injectNames.map(async (name) => ({
        path: name,
        bytes: await (await fetch(`${this.assetBaseUrl}inject/${name}`)).arrayBuffer(),
      })),
    );

    // ectt→cm-super map lines (font fix — scripts/vendor-busytex.sh).
    const fragmentRes = await fetch(`${this.assetBaseUrl}ectt-map-fragment.txt`);
    const mapAppend = fragmentRes.ok ? await fragmentRes.text() : "";

    onProgress?.(total, total, "Iniciando motor…");
    this.worker = new Worker(`${this.assetBaseUrl}uecetexlive.worker.js`);
