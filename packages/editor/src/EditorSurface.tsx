...
export interface EditorSurfaceProps {
  ...
  /** Norma de citação (parse/serialize/schema) — default ABNT. */
...
export function EditorSurface({
  ...
  const profileRef = useRef(citationProfile);
  profileRef.current = citationProfile;
  ...
  const editor = useEditor(
    {
      ...
      content: parseLatex(source, citationProfile).doc as never,
      editorProps: {
        attributes: {
      ...
      onCreate: ({ editor: current }) => {
        ...
        (window as unknown as Record<string, unknown>).__serialize = (json: unknown) =>
          serializeDoc(json as PMDoc, profileRef.current);
      },
      onUpdate: ({ editor: current }) => {
        if (applyingExternal.current) return;
        const json = current.getJSON() as unknown as PMDoc;
        onChangeRef.current(serializeDoc(json, profileRef.current));
      },
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    const { doc } = parseLatex(sourceRef.current, profileRef.current);
