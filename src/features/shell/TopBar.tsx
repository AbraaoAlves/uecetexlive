...
export interface TopBarProps {
  projectName: string;
  saveState: SaveState;
  /** Slot before the app name (rail toggle). */
  leading?: React.ReactNode;
  /** Compile controls slot — filled from Phase 2 on. */
  children?: React.ReactNode;
}

export function TopBar({ projectName, saveState, leading, children }: TopBarProps) {
  return (
    <header className="flex h-12 shrink-0 items-center gap-3 border-b bg-surface px-4">
      ...
      <span className="truncate text-ink-subtle text-sm" data-testid="work-title">
        {projectName}
      </span>
      <span
